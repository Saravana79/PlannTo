class Preference < ActiveRecord::Base
  ...
  def self.add_preference(user, search_type, params)
    ...
    Preference.clear_items(user, itemtype.id, search_attributes.collect(&:id))

    Preference.add_items(user, itemtype.id, search_attributes, params)
  ...
  def self.add_items(user, type, search_attributes, params)
    ...
    Preference.create(:itemtype_id => type, :user_id => user, :search_display_attribute_id => manufacturer_id.id, :value_1 => params["manufacturer"])
    end
    ...
    search_attributes.each do |search_attr|
      if search_attr.value_type == "Between"
        ...
        Preference.create(:itemtype_id => type, :user_id => user, :search_display_attribute_id => search_attr.id, :value_1 => params["#{attribute_field}"]) unless params["#{attribute_field}"].blank?
      end
    end
  end

  def self.clear_items(user, type, ids)
    Preference.delete_all(["user_id = ? and itemtype_id = ? and search_display_attribute_id in (?)", user, type, ids])
  end
