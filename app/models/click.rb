class Click < ActiveRecord::Base
  ...
  def self.save_click_data(url,request_referer,time,item_id,user,remote_ip,impression_id,publisher,vendor_id,source_type,temp_user_id)
    click = Click.find_by_impression_id_and_source_type_and_itemid_and_hosted_site_url_and_temp_user_id_and_vendor_id_and_click_url(impression_id, source_type, itemid, request_referer, temp_user_id, vendor_id, url )
    unless click
      click = Click.new
