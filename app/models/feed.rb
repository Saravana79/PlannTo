class Feed < ActiveRecord::Base
  ...
  def table_process()
    impression_missing = ImpressionMissing.where("updated_at > ? and count > ?", (self.last_updated_at.blank? ? Time.zone.now-2.days : self.last_updated_at), 1)
    # vendors_impression_missing = ImpressionMissing.where("updated_at > ? and req_type = ?", (self.last_updated_at.blank? ? Time.zone.now-2.days : self.last_updated_at), "vendor_page")
    vendors_impression_missing = []
    @impression_missing = (impression_missing + vendors_impression_missing).uniq
    admin_user = User.where(:is_admin => true).first
