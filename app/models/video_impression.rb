class VideoImpression < ActiveRecord::Base
  ...
  def self.add_video_impression_to_resque(param, remote_ip)
    ...
    impression_params = {:video_impression_id => impression_id, :type => param[:type], :ref_url => param[:ref_url], :item_id => param[:item_id], :time => Time.zone.now, :remote_ip => remote_ip, :advertisement_id => param[:ads_id], :winning_price => param[:winning_price_enc], :sid => param[:sid]}.to_json
