...
class Cycle < Product
  ...
  searchable :auto_index => true, :auto_remove => true  do
    ...
    float :rating_search_result do |item|
      ...
      (((item.item_rating.review_count/(item.item_rating.review_count + configatron.rating_m_for_cycle.to_i )) * item.item_rating.average_rating.to_f) + ((configatron.rating_m_for_cycle.to_i/(item.item_rating.review_count+configatron.rating_m_for_cycle.to_i))*avearge_rating_of_average )).to_f rescue 0.0
    end
