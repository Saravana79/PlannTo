class ItemAdDetail < ActiveRecord::Base
    ...
    if $redis.get("ad_details_for_items_is_running").to_i == 0
      ...
      begin
        ...
        items.each do |each_item|
          ...
          item_ad_detail = ItemAdDetail.where(:item_id => each_item.item_id).first_or_initialize
  ...
  def self.update_clicks_and_impressions_for_ad_details(log, batch_size=2000, time)
    ...
    begin
      ...
      impressions.each do |each_imp|
        ...
        item_ad_detail = ItemAdDetail.where(:item_id => each_imp.item_id).first_or_initialize
    ...
    @clicks.each do |each_click|
      ...
      item_ad_detail = ItemAdDetail.where(:item_id => each_click.item_id).first_or_initialize
    ...
    @orders.each do |each_order|
      ...
      item_ad_detail = ItemAdDetail.where(:item_id => each_order.item_id).first_or_initialize
  ...
  def self.update_clicks_and_impressions_for_ad_details_from_mongodb(log, batch_size=2000, time)
    ...
    results.each do |each_key, each_result|
      each_key
      item_ad_detail = ItemAdDetail.where(:item_id => each_key).first_or_initialize
