  ...
  def self.update_ad_details_for_contents(log, batch_size=2000)
    ...
    begin
      ...
      contents.each do |each_content|
        content_ad_detail = ContentAdDetail.where(:url => each_content.url).first_or_initialize
...
where ai.impression_time >= '#{date_for_query}' group by hosted_site_url order by count(*) desc"
    ...
    begin
      ...
      impressions.each do |each_imp|
        content_ad_detail = ContentAdDetail.where(:url => each_imp.url).first_or_initialize
    ...
    @clicks.each do |each_click|
      content_ad_detail = ContentAdDetail.where(:url => each_click.url).first_or_initialize
    ...
    @orders.each do |each_order|
      content_ad_detail = ContentAdDetail.where(:url => each_order.url).first_or_initialize
