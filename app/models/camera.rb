    ...
    float :rating_search_result do |item|
     if !(item.item_rating.nil?)
      ...
      diff_months = (current_date.year * 12 + current_date.month) - (lunch_date.year * 12 +  lunch_date.month) rescue 0
       ...
       rating = (((item.item_rating.review_count/(item.item_rating.review_count + configatron.rating_m_for_camera.to_f )) * item.item_rating.average_rating.to_f) + ((configatron.rating_m_for_camera.to_f/(item.item_rating.review_count+configatron.rating_m_for_camera.to_f))*avearge_rating_of_average )).to_f rescue 0.0
