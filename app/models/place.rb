class Place < Item
  ...
  def latitude
    lat = self.attribute_values.where(:attribute_id => 415).first
    if lat.blank?
      nil
    else
      lat.value.to_d
  ...
  def longitude
    lat = self.attribute_values.where(:attribute_id => 416).first
    if lat.blank?
      nil
    else
      lat.value.to_d
  ...
  def self.city_state_place_process
    ...
    ["Country", "State", "City", "Place"].each do |type|
      itemtype = Itemtype.where(:itemtype => type).first_or_initialize
    ...
    #create attributes
    latitude = Attribute.where(:name => "Latitude").first_or_initialize
    latitude.attribute_type = "String"
    latitude.category_name = "Location"
    latitude.save!
    longitude = Attribute.where(:name => "Longitude").first_or_initialize
    longitude.attribute_type = "String"
    longitude.category_name = "Location"
    longitude.save!
    place_series = Attribute.where(:name => "PlaceSeries").first_or_initialize
    place_series.attribute_type = "String"
    place_series.category_name = "Location"
    place_series.save!
    place_code = Attribute.where(:name => "PlaceCode").first_or_initialize
    place_code.attribute_type = "String"
    place_code.category_name = "Location"
    place_code.save!
    reference = Attribute.where(:name => "Reference ID").first_or_initialize
    reference.attribute_type = "String"
    reference.category_name = "General"
    reference.save!
    code = Attribute.where(:name => "Code").first_or_initialize
    code.attribute_type = "String"
    code.category_name = "Location"
    code.save!

    #create country
    country = Country.where(:name => "India").first_or_initialize
    ...
    place_details.each do |place_detail|
      if !place_detail[10].blank? && !place_detail[11].blank?
        ...
        existing_item = Item.where(:name => place_name).first
        ...
        if !existing_item.blank? && [State,City].include?(existing_item.class)
          ...
          if !latitude_val.blank?
            s_c_lat = AttributeValue.where(:attribute_id => latitude.id, :item_id => existing_item.id).first_or_initialize
            s_c_lat.update_attributes(:value => latitude_val)
          end

          # create or update longitude
          if !longitude_val.blank?
            s_c_lat = AttributeValue.where(:attribute_id => longitude.id, :item_id => existing_item.id).first_or_initialize
            s_c_lat.update_attributes(:value => longitude_val)
          end

          # create or update place code series
          if !place_series_val.blank?
            s_c_place_series = AttributeValue.where(:attribute_id => place_series.id, :item_id => existing_item.id).first_or_initialize
            s_c_place_series.update_attributes(:value => place_series_val)
          end

          # create or update place code
          if !place_code_val.blank?
            s_c_place_code = AttributeValue.where(:attribute_id => place_code.id, :item_id => existing_item.id).first_or_initialize
        ...
        place = Place.where(:name => place_name).first_or_initialize #TODO: Create duplicate if city is different
        ...
        if place.save(:validate => false)
          ...
          if !place_reference_id.blank?
            ref = AttributeValue.where(:attribute_id => reference.id, :item_id => place.id).first_or_initialize
            ref.update_attributes(:value => place_reference_id)
          end

          # create or update latitude
          if !latitude_val.blank?
            lat = AttributeValue.where(:attribute_id => latitude.id, :item_id => place.id).first_or_initialize
            lat.update_attributes(:value => latitude_val)
          end

          # create or update longitude
          if !longitude_val.blank?
            lat = AttributeValue.where(:attribute_id => longitude.id, :item_id => place.id).first_or_initialize
            lat.update_attributes(:value => longitude_val)
          end

          # create or update place code series
          if !place_series_val.blank?
            s_c_place_series = AttributeValue.where(:attribute_id => place_series.id, :item_id => place.id).first_or_initialize
            s_c_place_series.update_attributes(:value => place_series_val)
          end

          # create or update place code
          if !place_code_val.blank?
            s_c_place_code = AttributeValue.where(:attribute_id => place_code.id, :item_id => place.id).first_or_initialize
            s_c_place_code.update_attributes(:value => place_code_val)
          end
          ...
          begin
            ...
            relation = Itemrelationship.where(:item_id => place.id, :relationtype => "Place").first_or_initialize
            relation.update_attributes(:relateditem_id => relateditem_id)
          rescue Exception => e
