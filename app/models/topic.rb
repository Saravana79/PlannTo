class Topic < Item
  searchable :auto_index => true, :auto_remove => true  do
   text :name , :boost => 4.0,  :as => :name_ac do |item|
      item.name.gsub("_","")
    end 
    string :name do |item|
      item.name.gsub("_", " ")
  ...
  def self.topic_clouds(item_type)
     topics = ActiveRecord::Base.connection.execute("select i.id, name, count(*) from items i left outer join  item_contents_relations_cache c  on i.id = c.item_id inner join topic_itemtype_relations ti on ti.item_id = i.id  where  i.type ='Topic' and ti.itemtype_id = #{item_type.id} group by i.id")
