...
#  end
  ...
  def self.get_related_items(item, limit, includ=false)
    ...
    #end
    # related_item_ids = RelatedItem.where(:item_id => item.id).collect(&:related_item_id)
    return self.where(:id => related_item_ids).includes(:item_rating,:attribute_values,:cargroup).uniq{|x| x.cargroup}.first(limit) if item.type == Itemtype::CAR
    return self.where(:id => related_item_ids).includes(:item_rating,:attribute_values,:cargroup).first(limit)
  end
