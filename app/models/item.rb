...
def populate_pro_con
 ...
 def get_url()
  ...
  def self.update_item_details(log, batch_size=2000)
    ...
    query_to_set_null = "select distinct itemid from itemdetails where last_verified_date > '#{1.day.ago}' and status not in (1) and iserror != 1 and site in (9861,9882,9874,26351,73017,75798) and itemid not in (select itemid from itemdetails where last_verified_date > '#{1.day.ago}' and status  in (1) and iserror != 1 and site in (9861,9882,9874,26351,73017,75798)) order by itemid desc"
