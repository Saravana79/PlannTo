...
def populate_pro_con
 ...
 def get_url()
  ...
  def self.update_item_details(log, batch_size=2000, for_one_month="false")
    ...
    query_to_set_null = "select distinct itemid from itemdetails where last_verified_date > '#{date_range}' and status not in (1) and iserror != 1 and site in (9861,9882,9874,26351,73017,75798,76201,77161) and itemid not in (select itemid from itemdetails where last_verified_date > '#{date_range}' and status  in (1) and iserror != 1 and site in (9861,9882,9874,26351,73017,75798,76201,77161)) order by itemid desc"
    ...
    if ["type_4", "type_5"].include?(ad_template_type)
      if ["300","728","336_280"].include?(suitable_ui_size)
        ...
        if used_cars
          ...
          sliced_item_details = item_details.each_slice(4)
        else
              ...
              if already_exist == false
                if item_ids.blank?
                  ...
                  if apartment_domains.include?(domain)
                    ...
                    itemtype_id = 33
                    rk = 5
                  elsif ["click.in", "trovit.co.in", "locanto.in","olx.in"].include?(domain)
  ...
  def self.check_if_already_exist_in_user_visits(source_categories, user_id, url, url_prefix="users:last_visits")
    ...
    p "$redis.lrange('#{url_prefix}:#{user_id}',0,-1)"
  ...
  def self.get_loop_items_from_amazon_api(keyword, geo, browse_node, sort, valid_item_names)
    ...
    loop_items = loop_items.sample(10)
  ...
  def self.process_response(res)
    ...
    if !item.blank?
      ...
      category_item_detail.price = item.get_element("Offer/OfferListing/Price").get("FormattedPrice") rescue ""
  ...
  def self.get_random_id_from_item_ids(ad, item_ids, vendor_id=nil)
    item_ids_arr = item_ids.to_s.split(",")
    alt_item_ids = item_ids.to_s.gsub(",", "_")
    itemdetails_rand_id = ""

    # item = item_ids.blank? ? nil : Item.where(:id => item_ids.to_s.split(",")).first
    vendor_ids = ad.blank? ? [vendor_id] : [ad.vendor_id]
    vendor_ids = vendor_ids.compact
    if !alt_item_ids.blank?
      itemdetails_count = $redis.get("itemdetails_count:#{alt_item_ids}")

      if itemdetails_count.blank?
        items = Item.where(:id => item_ids_arr)
  ...
  def self.get_price_text_from_url(url, publisher, page_type="")
    ...
    unless url.nil?
      ...
      if tempurl.include?("m.mouthshut.com")
  ...
  def self.find_item_names_from_url(url=nil)
    ...
    [*0..3].each do |each_val|
      term = node.css(".hub-caption p")[each_val].content rescue ""
...
#term = term.to_s.split(/\W+/).delete_if{|x| (x =~ /\D/).blank? }.join(' ')
  ...
  def create_item_ad_detail
    ...
    new_item_ad_detail = ItemAdDetail.where(:item_id => self.id).first_or_initialize # new_item_ad_detail
    if old_item.blank?
