class CookieMatchingProcessEnqueue
  extend HerokuResqueAutoScale if Rails.env.production?
  extend Resque::Plugins::Retry
  @queue = :cookie_matching_process_enqueue

  def self.perform(method_name, actual_time)
    log = Logger.new 'log/cookie_matching_process_enqueue.log'
    #begin
    log.debug "********** Start Processing CookieMatchingProcessEnqueue **********"
    log.debug "********** Actual Time to Start #{actual_time.to_time.strftime('%b %d,%Y %r')} **********"

    now_time = actual_time.to_time

    CookieMatch.send(method_name)
    ...
    #end
    log.debug "********** End Processing CookieMatchingProcessEnqueue **********"
    log.debug "\n"
  end
end
