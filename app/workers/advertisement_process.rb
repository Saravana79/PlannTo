class AdvertisementProcess
  ...
  def self.perform(method_name, actual_time)
    ...
    begin
      ...
      log.debug "********** Calculate ecpm Started at - #{Time.now.strftime('%b %d,%Y %r')} **********"
      advertisements = Advertisement.send(method_name)
      log.debug "********** Calculate ecpm Completed at - #{Time.now.strftime('%b %d,%Y %r')} - #{advertisements} Advertisements updated **********"
    rescue Exception => e
