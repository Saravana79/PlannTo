class SourceItemUpdateWithAmazonEc
  extend HerokuResqueAutoScale if Rails.env.production?
  extend Resque::Plugins::Retry
  @queue = :source_item_update_process_with_amazon_ec

  def self.perform(method_name, actual_time)
    log = Logger.new 'log/source_item_process.log'
    #begin
      log.debug "********** Start Processing SourceItem with amazon ec details **********"
      ...
      log.debug "********** Article Content Creation Completed At - #{Time.zone.now.strftime('%b %d,%Y %r')} - #{source_items} sourceitems updated with amazon ec **********"
    #rescue => e
