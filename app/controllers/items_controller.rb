...
#    end
  ...
  def compare
    ...
    unless @ids.blank? || params[:ids] == ""
      ...
      @items = Item.includes([:item_attributes => :attribute_comparison_list], :contents).where("attribute_values.item_id in (?) and items.itemtype_id = ?", @ids, @item1.itemtype_id)
      @attributes = @items.collect{|i| i.item_attributes}.flatten.uniq
      @ids[0] = "0" if @ids[0] == ""
      @content = @items.where("contents.sub_type in ('Reviews','Deals','Lists')").select("contents.*")
      @attribute_comparision_lists = @attributes.collect(&:attribute_comparison_list).compact            
