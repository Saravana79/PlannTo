...
#    end
  ...
  def compare
    ...
    unless @ids.blank? || params[:ids] == ""
      ...
      @items = Item.includes(:attribute_values).find(@ids)
      ...
      @attribute_comparision_lists = AttributeComparisonList.where("attribute_id in (?) and itemtype_id = ?", attribute_ids, @item1.itemtype_id)
