  ...
  def index
   ...
   if params[:object_type] and params[:itemtype]
      ...
      if params[:object_type] == "item" 
        ...
        @pages_to_visit1+= item_type.contents.where('status = ?',1).collect{  |content| {:url =>  base_url + "/contents/" + content.id.to_s,  :updated_at => I18n.l(content.updated_at.nil?? Time.now : content.updated_at , :format => :w3c)} }
