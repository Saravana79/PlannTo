...
class HistoryDetailsController < ApplicationController
  ...
  def index
    ...
    if params[:id].present?
      ...
      click_params =  {:url => url, :request_referer => ref_url, :time => Time.zone.now, :item_id => item_id, :user => current_user.blank? ? nil : current_user.id, :remote_ip => request.remote_ip, :impression_id => @impression_id,
