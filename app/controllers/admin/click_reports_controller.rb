class Admin::ClickReportsController < ApplicationController
  ...
  def index
    ...
    @clicks = Click.where("publisher_id=? and timestamp >=? and timestamp <=?",publisher_id,@start_date.to_date,@end_date.to_date + 1.days).order('created_at desc').paginate(:per_page => 1,:page => params[:page])
