class Admin::ClickReportsController < ApplicationController
  ...
  def index
   ...
   @impressions = AddImpression.where("publisher_id=? and DATE(impression_time) >=? and DATE(impression_time) <=?",publisher_id,@start_date.to_date,@end_date.to_date).order('created_at desc').paginate(:per_page => 10,:page => params[:page])
   @impressionscount  = @impressions.count
  @clicks = Click.where("publisher_id=? and DATE(timestamp) >=? and DATE(timestamp) <=?",publisher_id,@start_date.to_date,@end_date.to_date).order('created_at desc').paginate(:per_page => 10,:page => params[:page])
