class Admin::ImpressionReportsController < ApplicationController
 ...
 def index
   ...
   @impressions = AddImpression.all.paginate(:per_page => 10,:page => params[:page])
   @clicks = Click.where("publisher_id=? and timestamp >=? and timestamp <=?",publisher_id,@start_date.to_date,@end_date.to_date).paginate(:per_page => 10,:page => params[:page])
 end
 end
