class Admin::PaymentReportsController < ApplicationController
  ...
  def index
    ...
    if current_user.is_admin?
      ...
      @publisher = Publisher.where(:id => user_relationship.relationship_id).last unless user_relationship.blank?
    end
  ...
  def edit
    @publishers = Publisher.all
    @payment_report = PaymentReport.where(:id => params[:id]).last
  ...
  def show
    @payment_report = PaymentReport.where(:id => params[:id]).last
    @publisher = Publisher.where(:id => @payment_report.publisher_id).last
    return redirect_to admin_payment_reports_path if @payment_report.blank?
