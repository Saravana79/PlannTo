...
// Localize jQuery variable
    ...
    function getScriptUrl() {
        ...
        for (var i = 0; i < scripts.length; i++)
        {
            ...
            if (src.indexOf(domain+"/javascripts/plannto.book_price.widget.js") != -1)
            {
...
//    event.preventDefault();
    ...
    function planntowtbdivcreation(item_ids,show_details,path, element_id, parentdivid,pathname,show_price,show_offer, sort_disable)
    {
        ...
        url = url_protocol + "//"+domain + SubPath + "?item_ids="+item_ids+"&price_full_details="+show_details+"&show_offer="+show_offer+"&show_price="+show_price+ "&path=" + path + "&ref_url="+pathname+"&doc_title-"+doc_title+"&sort_disable="+sort_disable+"&is_test="+is_test+"&page_type="+page_type+"&callback=?"
