...
task :related_items => :environment do
  Item.find_by_sql("select distinct item_id from item_contents_relations_cache INNER JOIN contents ON item_contents_relations_cache.content_id = contents.id where contents.updated_at > '#{2.days.ago}' and contents.sub_type in ('Comparisons')").each do |item|
    ...
    unless related_content_ids.blank?
      ...
      related_item_ids.uniq.each do |ri|
        # if RelatedItem.where(:item_id => item.item_id,:related_item_id => ri).blank?
...
task :related_items_with_count, [:all_item] => :environment do |t, args|
  ...
  begin
    ...
    items.each do |item|
      ...
      query = "select #{item.item_id} as item_id, item_id as related_item_id,count(*) as variance from item_contents_relations_cache where item_id != #{item.item_id} and content_id in (select contents.id from contents inner join item_contents_relations_cache ir on contents.id=ir.content_id
               and ir.item_id = #{item.item_id} where contents.itemtype_id = '#{item.itemtype_id}' and contents.sub_type in ('Comparisons')) group by item_id order by count(*) desc limit 20"
