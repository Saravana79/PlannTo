desc 'save related items in related items table'
    task :related_items => :environment do
      Item.find_by_sql("select distinct item_id from item_contents_relations_cache INNER JOIN contents ON item_contents_relations_cache.content_id = contents.id where contents.sub_type in ('Comparisons')").each do |item|
